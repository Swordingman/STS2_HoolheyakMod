...
internal static class TriggerErudition
{
    public static async Task Trigger(PlayerChoiceContext choiceContext, Creature? owner)
    {
        ...
        int finalDamage = analysis * multiplier;
        ...
        var forbidden = owner.GetPower<ForbiddenKnowledgeEruPower>();
        if (forbidden != null)
        ...
        var covenant = owner.GetPower<CovenantStrengthPower>();
        if (covenant != null)
            await covenant.Trigger(choiceContext);

        if (owner.Player != null)
            await ContingencyPlan.ReturnFromDiscard(choiceContext, owner.Player, true);
    }
